...
const MASTER_SIDE_CONFIRMED_TITLES = new Set([
  'Запись завершена',
  'Клиент подтвердил выполнение',
]);

/** Уведомления кабинета мастера не должны открываться как клиентская запись. */
function isMasterSideClientNotification(item: MeNotificationRow): boolean {
  const title = item.title.trim();
  if (MASTER_SIDE_CONFIRMED_TITLES.has(title)) return true;
  if (item.type === 'appointment_new') return true;
  if (item.body.trim().startsWith('Клиент:')) return true;
  return false;
}
...
export function resolveClientNotificationAction(
  ...
  if (isMasterSideClientNotification(item)) return null;
