...
import { MASTER_PROFILE_REPORT_REASONS } from '../../../features/masters/api/masterProfileReportApi';
import {
  getProfileReports,
  updateProfileReportStatus,
} from '../api/platformAdminApi';
import type { ProfileReportAdmin } from '../api/platformAdmin.types';
...
function reasonLabel(code: string): string {
  return MASTER_PROFILE_REPORT_REASONS.find((r) => r.code === code)?.label ?? code;
}

export function PlatformAdminProfileReportsTab() {
  const [filter, setFilter] = useState<StatusFilter>('pending');
  const [items, setItems] = useState<ProfileReportAdmin[]>([]);
  ...
  const load = useCallback(async (offset = 0) => {
    ...
    setError(null);
    try {
      const res = await getProfileReports(filter, { offset });
      setTotal(res.total);
  ...
  async function setStatus(id: string, status: 'in_review' | 'closed' | 'rejected', comment?: string) {
    setBusyId(id);
    try {
      await updateProfileReportStatus(id, status, comment);
      setRejectId(null);
      setRejectComment('');
      setCloseId(null);
      setCloseComment('');
    ...
    <div>
      ...
      {!loading && !error && items.length === 0 ? (
        <PlatformAdminEmpty
          ...
          text={
            filter === 'pending'
              ? 'Когда клиент пожалуется на профиль мастера, жалоба появится здесь.'
            ...
            <PlatformAdminCard key={req.id} className="space-y-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  {req.reporterName ? (
                    <p className="mt-1 text-[13px] text-[#6B7280]">От: {req.reporterName}</p>
                  ) : null}
                  <a
                    href={req.profileUrl}
                    target="_blank"
