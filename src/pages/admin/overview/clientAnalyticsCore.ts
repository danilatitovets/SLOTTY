/** Frontend mirror of server clientAnalyticsCore — единый слой расчёта клиентской аналитики. */

function isCompletedClientVisit(status: string): boolean {
  return status === 'completed';
}

function preferAnalyticsDisplayName(current: string, next: string): string {
  ...
  if (/^\+?\d[\d()-]{5,}$/.test(cur.replace(/\s/g, '')) && !/^\+?\d[\d()-]{5,}$/.test(nxt.replace(/\s/g, ''))) {
    return nxt;
  }
...
/** Пустой снимок для live-режима при ошибке API (без demo fallback). */
export function emptyClientAnalytics(): ClientAnalyticsSnapshot & {
  visitsPerDay: [];
  ...
  return {
    ...
    clientsPerDay: [],
    visitsPerDay: [],
    chartIsTruncated: false,
    hasData: false,
    newClientsDelta: 0,
    repeatClientsDelta: 0,
    totalClientsDelta: 0,
  };
}

if (import.meta.env.DEV) {
  (globalThis as typeof globalThis & { buildClientAnalyticsDebugSnapshot?: typeof buildClientAnalyticsDebugSnapshot }).buildClientAnalyticsDebugSnapshot =
